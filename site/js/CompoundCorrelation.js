function draw(data) {
    ...
    if(data==null) {
    	d3.select('#log')
            .append('p')
                .text(compoundID+' could not be found; please go back and check your entry')
                .style('color','red')
                .style('font-weight','bold');
    } else {
	...
	//      create chart
	    ...
	    chart.selectAll('circle')
		    ...
		    .append('circle')
		        ...
		        .attr('id',function(d) { return 'id'+d.compound_concentration_experiment_type2.replace('.','_') })
	...
	//	                    hide_data(); // too unstable for now
	            ...
	            .on('mouseover.tooltip',function(d,i) {
	            		d3.select('text#id'+d.compound_concentration_experiment_type2.replace('.','_')).remove();
	                    chart.append('text')
	                        .text(d.compound_name2)
	                        .attr('x',x_scale(i+1)+10)
	                        .attr('y',y_scale(d.Correlation)+5)
	                        .attr('id','id'+d.compound_concentration_experiment_type2.replace('.','_'));
	                })
	            .on('mouseout.tooltip',function(d) {
	            		d3.select('text#id'+d.compound_concentration_experiment_type2.replace('.','_'))
	                        .transition()
	                        .duration(500)
	                        .style('opacity',0)
	                        .remove();
	...
	//      create a row for each object in the data
	    var rows = tbody.selectAll("tr")
	        ...
	        .append("tr")
	            ...
	            .on('click',function(d) { window.open('../results/compound?ID='+d.compound_id2); })
	...
	//    create a cell in each row for each column
	    var columns = ['compound_id2','compound_name2','Correlation']
...
//    define brush functions
    ...
    function brushend() {
        if (brush.empty()) {
            ...
            chart.selectAll("circle")
                .style('fill','grey')
                .filter(function(d,i) { return x_scale(i+1) >= e[0][0] && x_scale(i+1) <= e[1][0]
                		&& d.Correlation >= e[0][1] && d.Correlation <= e[1][1]
                		} )
                .style('fill',null)
                .each(show_data);
        }
    ...
    function show_data(c) {
        var geneid = c.compound_concentration_experiment_type2;
        ...
        tbody.selectAll('tr')
            .filter(function(d) { return d.compound_concentration_experiment_type2==geneid } )
